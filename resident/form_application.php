...
} catch (PDOException $e) { die("Error resolving Resident ID: " . $e->getMessage()); }
...
try {
    // ---------------------------------------------------------
    // LOGIC A: HANDLE FORM SUBMISSION
    // ---------------------------------------------------------
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['submit_application'])) {
        ...
        if (!empty($_FILES['profile_image']['name'])) {
            ...
            if($old_p_row = $old_file_p_q->fetch(PDO::FETCH_ASSOC)){ $profile_image_path = $old_p_row['profile_image_path']; }
        }
        ...
        if (!empty($_FILES['valid_id_image']['name'])) {
            ...
            if($old_row = $old_file_q->fetch(PDO::FETCH_ASSOC)){ $valid_id_path = $old_row['valid_id_path']; }
        }
        ...
        if ($result) {
            ...
            if (isset($_POST['siblings']) && is_array($_POST['siblings'])) {
                ...
                foreach ($_POST['siblings'] as $sib) {
                    if (!empty($sib['name'])) { 
                        ...
                        $s_name = toUpper($sib['name']); $s_educ = toUpper($sib['education'] ?? ''); $s_occ = toUpper($sib['occupation'] ?? '');
            ...
            if (isset($_POST['children']) && is_array($_POST['children'])) {
                ...
                foreach ($_POST['children'] as $child) {
                    if (!empty($child['name'])) {
                        ...
                        $c_name = toUpper($child['name']); $c_occ = toUpper($child['occupation'] ?? ''); $c_educ = toUpper($child['education'] ?? '');
            ...
            exit;
        } else { echo "<script>alert('Database Execution Failed.');</script>"; }
    }

    // ---------------------------------------------------------
    // LOGIC B: CHECK STATUS
    // ---------------------------------------------------------
    ...
    if ($app_data = $stmt_check->fetch(PDO::FETCH_ASSOC)) {
        ...
        if (isset($_SESSION['user_type']) && $_SESSION['user_type'] == 'resident') {
            $has_application = true; $application_status = 'Verified'; 
        }
    }

    $is_editing = false;
    if(isset($_GET['action']) && $_GET['action'] == 'edit' && ($application_status == 'Rejected' || $application_status == 'Declined')){
        $has_application = false; $is_editing = true;       
    }

} catch(PDOException $e) { die("<div style='color:red;'>Database Error: " . $e->getMessage() . "</div>"); }

function getVal($key, $data){ global $is_editing; return ($is_editing && isset($data[$key])) ? htmlspecialchars($data[$key]) : ''; }
function isSel($key, $val, $data){ global $is_editing; return ($is_editing && isset($data[$key]) && $data[$key] == $val) ? 'selected' : ''; }
?>
...
<style>
  ...
  .ui-card { background-color: var(--card-bg); border: 1px solid var(--border-color); border-radius: var(--radius); padding: 0; max-width: 1100px; margin: 0 auto; box-shadow: 0 4px 20px rgba(0,0,0,0.4); }
  ...
  .header-badge { background-color: rgba(59, 130, 246, 0.15); color: var(--accent-color); padding: 6px 14px; border-radius: 20px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase; border: 1px solid rgba(59, 130, 246, 0.3); }
  .nav-tabs { border-bottom: 1px solid var(--border-color); margin-bottom: 30px; }
  .nav-tabs .nav-link { color: var(--text-muted); border: none; border-bottom: 2px solid transparent; background: transparent; padding: 12px 20px; font-weight: 500; }
  ...
  .form-control:focus { background-color: var(--input-bg); color: var(--text-main); border-color: var(--accent-color); }
  ...
  .btn-primary { background-color: var(--accent-color); border-color: var(--accent-color); font-weight: 600; padding: 10px 25px; border-radius: 6px; }
  ...
  .table-dark-custom th { border-bottom: 1px solid var(--border-color); color: var(--text-muted); font-size: 0.9rem; }
        ...
        <div class="ui-card status-container">
            ...
            <?php elseif ($application_status == 'Rejected' || $application_status == 'Declined'): ?>
                ...
                <?php if(!empty($admin_remarks)): ?>
                    <div class="alert alert-danger d-inline-block mt-3"><?= htmlspecialchars($admin_remarks) ?></div>
                <?php endif; ?>
                <div class="mt-5"><a href="form_application.php?action=edit" class="btn btn-outline-light">Edit Application</a></div>

            <?php else: ?>
                <div class="mb-4"><i class="fas fa-hourglass-half fa-5x" style="color: var(--warning);"></i></div>
                <h2 class="mb-3">Application Under Review</h2>
                <div class="p-3 rounded mt-3" style="background: rgba(245, 158, 11, 0.1); display: inline-block; color: var(--warning);">PENDING VERIFICATION</div>
                <div class="mt-5"><a href="dashboard.php" class="btn btn-outline-light btn-sm">Back to Dashboard</a></div>
            <?php endif; ?>
        </div>
      ...
      <?php else: ?>
        ...
        <div class="ui-card">
            <div class="ui-card-header">
                <h3>Residency Application Form</h3>
                <span class="header-badge"><?= $is_editing ? 'Resubmission' : 'New Application' ?></span>
            </div>

            <div class="ui-card-body">
              <p class="text-muted mb-4">Please fill out the form completely. All fields are required unless stated otherwise.</p>

              <form action="" method="POST" id="multiStepForm" enctype="multipart/form-data">
                ...
                <div class="tab-content pt-2">
                  ...
                  <div class="tab-pane fade show active" id="tab-applicant" role="tabpanel">
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="file" class="form-control" name="profile_image" id="profile_image_input" style="padding-top: 10px;" accept="image/*" <?= ($is_editing && !empty($app_data['profile_image_path'])) ? '' : 'required' ?>>
                                </div>
                                <div class="mt-2" id="profile_preview_container" style="<?= ($is_editing && !empty($app_data['profile_image_path'])) ? '' : 'display:none;' ?>">
                                     ...
                                     <img id="profile_preview" src="<?= ($is_editing && !empty($app_data['profile_image_path'])) ? $app_data['profile_image_path'] : '#' ?>" style="max-height: 150px; margin-top: 5px;">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                      <div class="col-md-3"><div class="form-group"><label>First Name</label><input type="text" class="form-control" name="first_name" value="<?= getVal('first_name',$app_data) ?>" required></div></div>
                      <div class="col-md-3"><div class="form-group"><label>Middle Name</label><input type="text" class="form-control" name="middle_name" value="<?= getVal('middle_name',$app_data) ?>" required></div></div>
                      <div class="col-md-3"><div class="form-group"><label>Last Name</label><input type="text" class="form-control" name="last_name" value="<?= getVal('last_name',$app_data) ?>" required></div></div>
                      <div class="col-md-3"><div class="form-group"><label>Suffix (Optional)</label><input type="text" class="form-control" name="suffix" value="<?= getVal('suffix',$app_data) ?>"></div></div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4"><div class="form-group"><label>Place of Birth</label><input type="text" class="form-control" name="pob" value="<?= getVal('birth_place',$app_data) ?>" required></div></div>
                    </div>

                    <div class="row">
                        <div class="col-md-3"><div class="form-group"><label>Nationality</label><input type="text" class="form-control" name="nationality" value="<?= getVal('nationality',$app_data) ?>" required></div></div>
                        <div class="col-md-3"><div class="form-group"><label>Civil Status</label><select class="form-control" name="civil_status" required><option value="">Select...</option><option <?= isSel('civil_status','Single',$app_data) ?>>Single</option><option <?= isSel('civil_status','Married',$app_data) ?>>Married</option><option <?= isSel('civil_status','Widowed',$app_data) ?>>Widowed</option><option <?= isSel('civil_status','Separated',$app_data) ?>>Separated</option></select></div></div>
                        <div class="col-md-3"><div class="form-group"><label>Religion</label><input type="text" class="form-control" name="religion" value="<?= getVal('religion',$app_data) ?>" required></div></div>
                        <div class="col-md-3"><div class="form-group"><label>Blood Type</label><select class="form-control" name="blood_type" required><option value="">Select...</option><option value="A+" <?= isSel('blood_type','A+',$app_data) ?>>A+</option><option value="B+" <?= isSel('blood_type','B+',$app_data) ?>>B+</option><option value="O+" <?= isSel('blood_type','O+',$app_data) ?>>O+</option><option value="AB+" <?= isSel('blood_type','AB+',$app_data) ?>>AB+</option><option value="UNKNOWN" <?= isSel('blood_type','UNKNOWN',$app_data) ?>>UNKNOWN</option></select></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-12"><div class="form-group"><label>Occupation</label><input type="text" class="form-control" name="occupation" value="<?= getVal('occupation',$app_data) ?>" required></div></div>
                    </div>

                    <div class="section-title"><i class="fas fa-map-marker-alt"></i> Address & Contact</div>
                    <div class="row">
                      <div class="col-md-3"><div class="form-group"><label>House No.</label><input type="text" class="form-control" name="house_number" value="<?= getVal('house_number',$app_data) ?>" required></div></div>
                      <div class="col-md-3"><div class="form-group"><label>Purok</label><input type="text" class="form-control" name="purok" value="<?= getVal('purok',$app_data) ?>" required></div></div>
                    </div>

                    <div class="row">
                        <div class="col-md-6"><div class="form-group"><label>Contact Number</label><input type="text" id="contact_number" class="form-control" name="contact_number" value="<?= getVal('contact_number',$app_data) ?>" placeholder="09XXXXXXXXX" maxlength="11" required></div></div>
                        <div class="col-md-6"><div class="form-group"><label>Email Address</label><input type="email" class="form-control" name="email_address" value="<?= getVal('email_address',$app_data) ?>" required></div></div>
                    </div>

                    <div class="section-title"><i class="fas fa-list"></i> Additional Information</div>
                    <div class="row">
                        <div class="col-md-3"><div class="form-group"><label>Voter Status</label><select class="form-control" name="voter" required><option value="">Select</option><option value="Yes" <?= isSel('voter_status','Yes',$app_data) ?>>Yes</option><option value="No" <?= isSel('voter_status','No',$app_data) ?>>No</option></select></div></div>
                        
                        <div class="col-md-3">
                            <div class="form-group"><label>PWD</label><select class="form-control" name="pwd" id="pwd_status" required><option value="">Select</option><option value="Yes" <?= isSel('pwd_status','Yes',$app_data) ?>>Yes</option><option value="No" <?= isSel('pwd_status','No',$app_data) ?>>No</option></select></div>
                        </div>
                        <div class="col-md-3" id="pwd_type_div" style="display: none;">
                            <div class="form-group"><label>Specify Disability</label><input type="text" class="form-control" name="pwd_type" id="pwd_type_input" placeholder="Type of Disability" value="<?= getVal('pwd_type',$app_data) ?>"></div>
                        </div>

                        <div class="col-md-3"><div class="form-group"><label>Single Parent</label><select class="form-control" name="single_parent" required><option value="">Select</option><option value="Yes" <?= isSel('single_parent_status','Yes',$app_data) ?>>Yes</option><option value="No" <?= isSel('single_parent_status','No',$app_data) ?>>No</option></select></div></div>
                        <div class="col-md-3"><div class="form-group"><label>Senior Citizen</label><select class="form-control" name="senior_citizen" required><option value="">Select</option><option value="Yes" <?= isSel('senior_status','Yes',$app_data) ?>>Yes</option><option value="No" <?= isSel('senior_status','No',$app_data) ?>>No</option></select></div></div>
                    </div>

                    <div class="section-title"><i class="fas fa-user-friends"></i> Parents Details</div>
                    <div class="row">
                        <div class="col-md-8"><div class="form-group"><label>Father's Name</label><input type="text" class="form-control" name="father_name" value="<?= getVal('father_name',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Father's Occupation</label><input type="text" class="form-control" name="father_occupation" value="<?= getVal('father_occupation',$app_data) ?>" required></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-4"><div class="form-group"><label>Father's Age</label><input type="number" class="form-control" name="father_age" value="<?= getVal('father_age',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Father's Birthday</label><input type="date" class="form-control" name="father_birthday" value="<?= getVal('fathers_bday',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Father's Education</label><select class="form-control" name="father_education" required><option value="">Select</option><option value="Elementary" <?= isSel('father_education','Elementary',$app_data) ?>>Elementary</option><option value="High School" <?= isSel('father_education','High School',$app_data) ?>>High School</option><option value="College" <?= isSel('father_education','College',$app_data) ?>>College</option><option value="Vocational" <?= isSel('father_education','Vocational',$app_data) ?>>Vocational</option><option value="None" <?= isSel('father_education','None',$app_data) ?>>None</option></select></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-8"><div class="form-group"><label>Mother's Name</label><input type="text" class="form-control" name="mother_name" value="<?= getVal('mother_name',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Mother's Occupation</label><input type="text" class="form-control" name="mother_occupation" value="<?= getVal('mother_occupation',$app_data) ?>" required></div></div>
                    </div>
                    <div class="row">
                        <div class="col-md-4"><div class="form-group"><label>Mother's Age</label><input type="number" class="form-control" name="mother_age" value="<?= getVal('mother_age',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Mother's Birthday</label><input type="date" class="form-control" name="mother_birthday" value="<?= getVal('mothers_bday',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Mother's Education</label><select class="form-control" name="mother_education" required><option value="">Select</option><option value="Elementary" <?= isSel('mother_education','Elementary',$app_data) ?>>Elementary</option><option value="High School" <?= isSel('mother_education','High School',$app_data) ?>>High School</option><option value="College" <?= isSel('mother_education','College',$app_data) ?>>College</option><option value="Vocational" <?= isSel('mother_education','Vocational',$app_data) ?>>Vocational</option><option value="None" <?= isSel('mother_education','None',$app_data) ?>>None</option></select></div></div>
                    </div>

                    <div class="section-title"><i class="fas fa-user-shield"></i> Guardian Info</div>
                    <div class="row">
                        <div class="col-md-6"><div class="form-group"><label>Guardian's Name</label><input type="text" class="form-control" name="guardian" value="<?= getVal('guardian_name',$app_data) ?>"></div></div>
                        <div class="col-md-6"><div class="form-group"><label>Guardian's Contact</label><input type="text" class="form-control" name="guardian_contact" value="<?= getVal('guardian_contact',$app_data) ?>"></div></div>
                    </div>
                  </div>
                  ...
                  <div class="tab-pane fade" id="tab-residency" role="tabpanel">
                    ...
                    <div class="row">
                        <div class="col-md-4"><div class="form-group"><label>How long as resident?</label> <select class="form-control" name="residency_months" required><option value="">SELECT</option><option value="below_6" <?= isSel('residency_duration','below_6',$app_data) ?>>Less than 6 months</option><option value="above_6" <?= isSel('residency_duration','above_6',$app_data) ?>>6 months or more</option></select></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Resident Since</label> <input type="date" class="form-control" name="resident_since" id="resident_since" value="<?= getVal('residence_since',$app_data) ?>" required></div></div>
                        <div class="col-md-4"><div class="form-group"><label>Years of Residing</label> <input type="text" class="form-control" name="years_of_living" id="years_of_living" placeholder="Auto-calculated..." value="<?= getVal('years_of_living',$app_data) ?>" readonly style="background-color: #2b303b; cursor: not-allowed;"></div></div>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="input-group-prepend"><span class="input-group-text"><i class="fas fa-upload"></i></span></div>
                                    <input type="file" class="form-control" name="valid_id_image" id="valid_id_image_input" style="padding-top: 10px;" accept="image/*" <?= ($is_editing && !empty($app_data['valid_id_path'])) ? '' : 'required' ?>>
                                </div>
                                <div class="mt-2" id="id_preview_container" style="<?= ($is_editing && !empty($app_data['valid_id_path'])) ? '' : 'display:none;' ?>">
                                    <small class="text-muted">Preview:</small><br>
                                    <img id="id_preview" src="<?= ($is_editing && !empty($app_data['valid_id_path'])) ? $app_data['valid_id_path'] : '#' ?>" style="max-height: 150px; margin-top: 5px;">
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="table-responsive"><table class="table table-bordered table-dark-custom"><thead><tr><th>Name</th><th>Birthdate</th><th>Age</th><th>Civil Status</th><th>Occupation</th><th>Highest Educational Attainment</th><th>Action</th></tr></thead><tbody id="children_tbody"></tbody></table></div>
                    ...
                    <div class="table-responsive"><table class="table table-bordered table-dark-custom"><thead><tr><th>Name</th><th>Age</th><th>Birthday</th><th>Civil Status</th><th>Education</th><th>Occupation</th><th>Action</th></tr></thead><tbody id="siblings_tbody"></tbody></table></div>
...
$(function(){
  ...
  $('#dob').on('change', function() {
      ...
      var today = new Date();
      if (isNaN(dob.getTime())) { $('#age').val(''); } else {
  ...
  function calculateResidency() {
      ...
      if (!startDateInput) { outputField.val(''); return; }
      ...
      if (years < 0) { outputField.val("Date is in the future"); return; }
      outputField.val((years + (years === 1 ? " Year" : " Years")) + " and " + (months + (months === 1 ? " Month" : " Months")));
  }
  $('#resident_since').on('change', calculateResidency);
  if($('#resident_since').val()){ calculateResidency(); }

  // --- Toggle PWD & Required Logic ---
  function togglePwd(){
      if($('#pwd_status').val() == 'Yes'){
          $('#pwd_type_div').show();
          $('#pwd_type_input').prop('required', true); // Make required if visible
      } else {
          $('#pwd_type_div').hide();
          $('#pwd_type_input').val(''); 
          $('#pwd_type_input').prop('required', false); // Not required if hidden
      }
  }
  ...
  function readURL(input, imgSelector, containerSelector) {
    if (input.files && input.files[0]) {
        ...
        reader.onload = function(e) { $(imgSelector).attr('src', e.target.result); $(containerSelector).show(); }
  ...
  function addChildRow(data){
    ...
    var $tr = $('<tr>');
    $tr.append('<td><input type="text" name="children['+idx+'][name]" class="form-control" value="'+(data.name||'')+'" required></td>');
    $tr.append('<td><input type="date" name="children['+idx+'][birthday]" class="form-control" value="'+(data.birthdate||'')+'" required></td>');
    $tr.append('<td><input type="number" name="children['+idx+'][age]" class="form-control" value="'+(data.age||'')+'" required></td>');
    $tr.append('<td><select name="children['+idx+'][civil_status]" class="form-control" required><option value="Single">Single</option><option value="Married">Married</option></select></td>');
  ...
  function addSiblingRow(data){
    ...
    var idx = Date.now() + Math.random().toString(36).substring(7);
    var $tr = $('<tr>');
    $tr.append('<td><input type="text" name="siblings['+idx+'][name]" class="form-control" value="'+(data.name||'')+'" required></td>');
    $tr.append('<td><input type="number" name="siblings['+idx+'][age]" class="form-control" value="'+(data.age||'')+'" required></td>');
    $tr.append('<td><input type="date" name="siblings['+idx+'][birthday]" class="form-control" value="'+(data.birthday||'')+'" required></td>');
    $tr.append('<td><select name="siblings['+idx+'][civil_status]" class="form-control" required><option value="Single">Single</option><option value="Married">Married</option></select></td>');
  ...
  // --- TAB VALIDATION (FIX FOR SUBMIT ISSUE) ---
  function updateFooterButtons(){
    var onApplicant = $('#tab-applicant').hasClass('active');
    if(onApplicant){ $('#btn-next').show(); $('#btn-submit').hide(); } 
    else { $('#btn-next').hide(); $('#btn-submit').show(); }
  }
  
  $('a[data-toggle="tab"]').on('shown.bs.tab', function (e) { updateFooterButtons(); });

  // CHECK VALIDITY BEFORE SWITCHING TABS
  $('#btn-next').on('click', function(){
      var tab1Inputs = document.querySelectorAll('#tab-applicant input, #tab-applicant select');
      var allValid = true;
      for (var i = 0; i < tab1Inputs.length; i++) {
          if (!tab1Inputs[i].checkValidity()) {
              tab1Inputs[i].reportValidity(); // Shows the browser validation bubble
              allValid = false;
              break; 
          }
      }
  ...
  // Gov Beneficiary Toggle & Required
  $('input[name="gov_beneficiary"]').on('change', function(){
    if($('#gov_yes').is(':checked')){ 
        $('#beneficiary_type_wrap').slideDown(); 
        $('#beneficiary_type').prop('required', true); // Make required
    } else { 
        $('#beneficiary_type_wrap').slideUp(); 
        $('#beneficiary_type').val(''); 
        $('#beneficiary_type').prop('required', false); // Not required
    }
  });
  // Initial check
  if($('input[name="gov_beneficiary"]:checked').val() == 'yes'){
      $('#beneficiary_type_wrap').show();
      $('#beneficiary_type').prop('required', true);
  }

  // --- Pre-fill ---
  <?php 
  if($is_editing){
      $stmtS = $pdo->prepare("SELECT * FROM resident_siblings WHERE resident_id = ?"); $stmtS->execute([$resident_id]); $sibs = $stmtS->fetchAll(PDO::FETCH_ASSOC);
      echo "var savedSibs = " . json_encode($sibs) . "; if(savedSibs){ savedSibs.forEach(function(s){ addSiblingRow(s); }); }";
      $stmtC = $pdo->prepare("SELECT * FROM resident_children WHERE resident_id = ?"); $stmtC->execute([$resident_id]); $kids = $stmtC->fetchAll(PDO::FETCH_ASSOC);
      echo "var savedKids = " . json_encode($kids) . "; if(savedKids){ savedKids.forEach(function(k){ addChildRow(k); }); }";
  }
  ?>
});
</script>
