...
try {
    ...
    $add_last_name = $_POST['add_last_name']; // We need this for the username
    ...
    // 1. Generate Username: Remove spaces from lastname + 2 random digits (10-99)
    $clean_lastname = str_replace(' ', '', $add_last_name); 
    $generated_username = $clean_lastname . rand(10, 99);

    // 2. Generate Password: 8 random digits (10000000 - 99999999)
    ...
    $stmt_user->execute([
        $generated_username, // New Custom Username
        $generated_password, // New 8-digit Password
        $user_type,
        $add_contact_number
    ]);
    ...
    // We send a delimiter '|' so Javascript can split it and show the password
    echo "SUCCESS|" . $generated_username . "|" . $generated_password;
