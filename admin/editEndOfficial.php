...
$sql_check_official = "SELECT official_end_status.*, official_end_information.*, position.position AS position_row FROM official_end_status 
INNER JOIN official_end_information ON official_end_status.official_id = official_end_information.official_id 
INNER JOIN position ON official_end_status.position = position.position_id
WHERE official_end_status.official_id = ?";
...
if(isset($edit_image)){
  ...
  $sql_check_image = "SELECT `image`, `image_path` FROM `official_end_information`  WHERE `official_id` = ?";
...
$sql_official = "UPDATE `official_end_information` SET 
`first_name`= ?,
`middle_name`= ?,
`last_name`= ?,
`age`= ?,
...
$sql_edit_official_status = "UPDATE `official_end_status` SET `voters` = ?, `senior` = ?, `pwd` = ?,`pwd_info` = ?, `single_parent` = ?, `term_from` = ?, `term_to` = ?, `position` = ? WHERE `official_id` = ?";
...
if($_POST['edit_first_name_check'] == 'true' || $_POST['edit_first_name_check'] === TRUE){
  ...
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) - FIRST NAME '.' ' .$official_id.' |' .' '. ' FROM '.$old_first_name.' TO '. $edit_first_name;
...
if($_POST['edit_last_name_check'] == 'true' || $_POST['edit_last_name_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) LAST NAME- '.' ' .$official_id.' |' .' '. ' FROM '.$old_last_name.' TO '. $edit_last_name;
  $status_activity_log = 'update';
...
if($_POST['edit_position_check'] == 'true' || $_POST['edit_position_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) POSITION - '.' ' .$official_id.' |' .' '. ' FROM '.$old_position.' TO '. $new_position;
  $status_activity_log = 'update';
...
if($_POST['edit_term_from_check'] == 'true' || $_POST['edit_term_from_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) START - '.' ' .$official_id.' |' .' '. ' FROM '.$old_term_from.' TO '. $edit_term_from;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_term_to_check'] == 'true' || $_POST['edit_term_to_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) END - '.' ' .$official_id.' |' .' '. ' FROM '.$old_term_to.' TO '. $edit_term_to;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_voters_check'] == 'true' || $_POST['edit_voters_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) VOTERS - '.' ' .$official_id.' |' .' '. ' FROM '.$old_voters.' TO '. $edit_voters;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_pwd_check'] == 'true' || $_POST['edit_pwd_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) PWD - '.' ' .$official_id.' |' .' '. ' FROM '.$old_pwd.' TO '. $edit_pwd;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_birth_date_check'] == 'true' || $_POST['edit_birth_date_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) BIRTH DATE - '.' ' .$official_id.' |' .' '. ' FROM '.$old_birth_date.' TO '. $edit_birth_date;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_birth_place_check'] == 'true' || $_POST['edit_birth_place_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) BIRTH PLACE - '.' ' .$official_id.' |' .' '. ' FROM '.$old_birth_place.' TO '. $edit_birth_place;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_middle_name_check'] == 'true' || $_POST['edit_middle_name_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) MIDDLE NAME - '.' ' .$official_id.' |' .' '. ' FROM '.$old_middle_name.' TO '. $edit_middle_name;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_suffix_check'] == 'true' || $_POST['edit_suffix_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) SUFFIX - '.' ' .$official_id.' |' .' '. ' FROM '.$old_suffix.' TO '. $edit_suffix;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_gender_check'] == 'true' || $_POST['edit_gender_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) GENDER - '.' ' .$official_id.' |' .' '. ' FROM '.$old_gender.' TO '. $edit_gender;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_civil_status_check'] == 'true' || $_POST['edit_civil_status_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) CIVIL STATUS - '.' ' .$official_id.' |' .' '. ' FROM '.$old_civil_status.' TO '. $edit_civil_status;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_religion_check'] == 'true' || $_POST['edit_religion_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) RELIGION - '.' ' .$official_id.' |' .' '. ' FROM '.$old_religion.' TO '. $edit_religion;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_nationality_check'] == 'true' || $_POST['edit_nationality_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) NATIONALITY - '.' ' .$official_id.' |' .' '. ' FROM '.$old_nationality.' TO '. $edit_nationality;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_municipality_check'] == 'true' || $_POST['edit_municipality_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) MUNICPALITY - '.' ' .$official_id.' |' .' '. ' FROM '.$old_municipality.' TO '. $edit_municipality;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_zip_check'] == 'true' || $_POST['edit_zip_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) ZIP - '.' ' .$official_id.' |' .' '. ' FROM '.$old_zip.' TO '. $edit_zip;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_barangay_check'] == 'true' || $_POST['edit_barangay_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) BARANGAY - '.' ' .$official_id.' |' .' '. ' FROM '.$old_barangay.' TO '. $edit_barangay;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_house_number_check'] == 'true' || $_POST['edit_house_number_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) HOUSE NUMBER - '.' ' .$official_id.' |' .' '. ' FROM '.$old_house_number.' TO '. $edit_house_number;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_street_check'] == 'true' || $_POST['edit_street_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) STREET - '.' ' .$official_id.' |' .' '. ' FROM '.$old_street.' TO '. $edit_street;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_address_check'] == 'true' || $_POST['edit_address_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) ADDRESS - '.' ' .$official_id.' |' .' '. ' FROM '.$old_address.' TO '. $edit_address;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_email_address_check'] == 'true' || $_POST['edit_email_address_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) EMAIL ADDRESS - '.' ' .$official_id.' |' .' '. ' FROM '.$old_email_address.' TO '. $edit_email_address;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_contact_number_check'] == 'true' || $_POST['edit_contact_number_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) CONTACT NUMBER - '.' ' .$official_id.' |' .' '. ' FROM '.$old_contact_number.' TO '. $edit_contact_number;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_fathers_name_check'] == 'true' || $_POST['edit_fathers_name_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) FATHERS NAME  - '.' ' .$official_id.' |' .' '. ' FROM '.$old_fathers_name.' TO '. $edit_fathers_name;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_mothers_name_check'] == 'true' || $_POST['edit_mothers_name_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) MOTHERS NAME  - '.' ' .$official_id.' |' .' '. ' FROM '.$old_mothers_name.' TO '. $edit_mothers_name;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_guardian_check'] == 'true' || $_POST['edit_guardian_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) GUARDIAN  - '.' ' .$official_id.' |' .' '. ' FROM '.$old_guardian.' TO '. $edit_guardian;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
...
if($_POST['edit_guardian_contact_check'] == 'true' || $_POST['edit_guardian_contact_check'] === TRUE){
  ...
  $date_activity = $now = date("j-n-Y g:i A");  
  $admin = strtoupper('ADMIN').':' .' '. 'UPDATED OFFICIAL(DELETED) GUARDIAN CONTACT  - '.' ' .$official_id.' |' .' '. ' FROM '.$old_guardian_contact.' TO '. $edit_guardian_contact;
  $status_activity_log = 'update';
  $sql_activity_log = "INSERT INTO activity_log (`message`,`date`,`status`)VALUES(?,?,?)";
  $stmt_activity_log = $con->prepare($sql_activity_log) or die ($con->error);
  $stmt_activity_log->bind_param('sss',$admin,$date_activity,$status_activity_log);
